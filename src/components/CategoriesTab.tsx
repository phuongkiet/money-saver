...
import { FolderHeart, Pencil, Check, AlertTriangle, Palette, PlusCircle } from 'lucide-react';
import { formatThousand, parseThousand } from '../utils/format';

export const CategoriesTab: React.FC = () => {
  const { categories, transactions, updateCategoryBudget, updateCategoryColor, addCategory, showToast } = useApp();
  const [editingId, setEditingId] = useState<string | null>(null);
  const [tempBudget, setTempBudget] = useState<string>('');
  const [showColorPickerId, setShowColorPickerId] = useState<string | null>(null);

  // Form states for adding new budget
  ...
  // Predefined color palette for fast selection
  const colorPalette = [
    '#8fae8d', // Sage Green iPhone 17
    '#e5c158', // Yellow
    '#57c5f7', // Light Blue
    '#f36e6e', // Red
    '#9d7bf5', // Purple
  ...
  // List of preselected icons for new budget creation
  const iconOptions = [
  ...
  const handleEditBudgetClick = (catId: string, currentBudget: number) => {
    setEditingId(catId);
    setTempBudget(formatThousand(currentBudget.toString()));
  };

  const handleSaveBudget = (catId: string) => {
    const parsed = parseThousand(tempBudget);
    if (parsed >= 0) {
      updateCategoryBudget(catId, parsed);
    }
    setEditingId(null);
  };
        ...
        <form onSubmit={handleAddBudgetSubmit} className="bg-white dark:bg-zinc-900 p-5 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-md space-y-4 animate-slide-down">
          ...
          <div className="space-y-3.5">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap gap-2.5 bg-zinc-50 dark:bg-zinc-950/40 p-2.5 rounded-xl border border-zinc-100 dark:border-zinc-850">
            ...
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-4 gap-2 bg-zinc-50 dark:bg-zinc-950/40 p-2.5 rounded-xl border border-zinc-100 dark:border-zinc-850">
                  ...
                  <button
                    ...
                    className={`p-2 rounded-lg border text-center flex flex-col items-center gap-1 transition-all ${
                      newBudgetIcon === icon.name
                        ? 'bg-[#8fae8d]/10 border-[#8fae8d] text-[#6f8d6d] font-semibold'
            ...
            <button
              type="submit"
              className="w-full py-3 bg-[#6f8d6d] hover:bg-[#5b755a] text-white text-xs font-bold font-vietnam rounded-xl shadow-md active:scale-95 transition-all mt-2"
            >
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center justify-between">
                ...
                {/* Right Actions: Color Customize or Budget Edit */}
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setShowColorPickerId(showColorPickerId === cat.id ? null : cat.id)}
                    className="p-2 text-zinc-400 hover:text-[#6f8d6d] bg-zinc-50 dark:bg-zinc-800/50 rounded-xl transition-all"
                  ...
                  {editingId === cat.id ? (
                    <button
                      ...
                      className="p-2 text-emerald-500 bg-emerald-50 dark:bg-emerald-950/20 rounded-xl transition-all"
                      title="Lưu"
                    >
                    ...
                    <button
                      onClick={() => handleEditBudgetClick(cat.id, cat.budget)}
                      className="p-2 text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl transition-all"
                      title="Sửa định mức"
                    >
                      <Pencil size={14} />
                    </button>
              ...
              {showColorPickerId === cat.id && (
                <div className="bg-zinc-50 dark:bg-zinc-800/40 p-3 rounded-2xl border border-zinc-100 dark:border-zinc-800 animate-slide-down">
                  <div className="text-[10px] font-bold font-vietnam text-zinc-400 dark:text-zinc-500 mb-2 uppercase">Chọn màu tùy chỉnh:</div>
                  <div className="flex flex-wrap gap-2.5">
                    {colorPalette.map(color => (
                      <button
                        ...
                        className={`w-6 h-6 rounded-full border-2 transition-transform hover:scale-110 active:scale-95 ${
                          cat.color === color ? 'border-zinc-700 dark:border-white' : 'border-transparent'
                        }`}
                        style={{ backgroundColor: color }}
                      ></button>
                    ))}
                ...
                {editingId === cat.id ? (
                  <div className="flex items-center gap-1.5 max-w-[150px]">
                    <input
                      ...
                      className="w-full bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-2.5 py-1 text-xs font-bold font-vietnam text-zinc-800 dark:text-zinc-100 focus:outline-none focus:ring-1 focus:ring-[#8fae8d]"
                      placeholder="0đ (không hạn mức)"
                    />
                    <span className="text-xs font-bold text-zinc-400 font-vietnam">đ</span>
                  </div>
                ) : (
